@include('pages.utilisateur.partials.nav')
<div id="bg_dashboard">
    <div class="container">
        <div class="compteur">
            <p id="nombre_cred">10</p>
            <p id="credits">crédits</p>
        </div>
        <div class="row dash">
            <div class="col-xs-6">
                <a id="lien_btn" href="{{url('acheter_sacs')}}">
                    <div class="cercle">
                        <img class="picto_btn img-responsive" src="/images/picto/acheter_sac.png">
                    </div>
                    <p class="calltoaction">
                    Acheter des sacs
                    </p>
                </a>
            </div>
            <div class="col-xs-6">
                <div class="cercle">
                    <img class="picto_btn img-responsive" src="/images/picto/qr-code.png">
                </div>
                <p class="calltoaction"><a href="{{url('user/qrcode/'.\Auth::user()->id)}}">QR code</a></p>
            </div>
        </div>
        <div class="row dash">
            <div class="col-xs-6">
                <div class="cercle">
                    <img class="picto_btn img-responsive" src="/images/picto/historique.png">
                </div>
                <p class="calltoaction">Historique<br>des transactions</p>
            </div>
            <div class="col-xs-6">
                <a id="lien_btn" href="{{url('trouver_magasin')}}">
                    <div class="cercle">
                        <img class="picto_btn img-responsive" src="/images/picto/trouver.png">
                    </div>
                    <p class="calltoaction">Trouver un <br> magasin</p>
                </a>
            </div>
        </div>

    </div>
</div>